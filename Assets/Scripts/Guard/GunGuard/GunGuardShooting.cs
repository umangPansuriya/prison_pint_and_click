...
namespace NPC.Guard
{
    public class GunGuardShooting : GunGuardState
    {
        private Transform _player;
        private Coroutine _fireCoolDownCoroutine;
        public GunGuardShooting(GunGuard guard, Transform player) : base(guard)
        {
        ...
        public void OnTick()
        {
            ...
            else
            {
                ...
                {
                    _guard.StateManager.SwitchStateTo(new GunGuardCatch(_guard, _player));
                    return;
                }
                else if (distance > _guard.ShootRange && distance < _guard.ChaseRange)
                {
                    _guard.StateManager.SwitchStateTo(new GunGuardChasing(_guard, _player));
                    return;
                }
            }
        ...
        {
            _guard.StateManager.SwitchStateTo(new GunGuardSearching(_guard, _player));
        }
